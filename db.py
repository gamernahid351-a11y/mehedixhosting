...
async def get_pool():
    ...
    if _pool is None:
        ...
        # Railway uses postgres:// but asyncpg needs host/port/etc
        # Also Railway requires SSL — build ssl context
        r = urlparse(db_url)

        ssl_ctx = None
        if IS_RAILWAY or "railway" in (r.hostname or "") or "neon" in (r.hostname or ""):
...
async def run_migrations():
    ...
    async with pool.acquire() as c:
        ...
        # Safe column migrations (existing deployments)
        safe_alters = [
        ...
        if user_count == 0:
            import bcrypt
            admin_email    = os.environ.get("ADMIN_EMAIL",    "admin@bdxhosting.com")
